...
public class PackageConfigHandler extends PluginConfigHandler {
    ...
    public Map handleConfiguration() {
        ...
        packageConfig.put("SKELETON_PACKAGE_ID", createConfigurationField("Skeleton Package ID", "0", false, true, true));
    ...
    public List handleValidateConfiguration(Map request) {
        ...
        Map packageIDMap = (Map) configMap.get("SKELETON_PACKAGE_ID");

        if (packageIDMap.get("value").equals("")) {
            Map idErrors = new HashMap();
            idErrors.put("key", "SKELETON_PACKAGE_ID");
            idErrors.put("message", "Skeleton Package ID cannot be empty");
